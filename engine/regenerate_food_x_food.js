// Regenerates every food×food templated note via the v4 engine.
// Identifies templated by signature (engine-output patterns); preserves
// any note that doesn't match (editorial / hand-curated).
'use strict';
const fs = require('fs');
const vm = require('vm');
...
// Templated-note signatures (engine-output patterns). Matches v3 + v4 output.
const TEMPLATED_SIGS = [
  ...
  /the cocktail closes the meal/,  // unlikely match in food×food but safety
  ...
  // Additional v3 closers that didn't match the v4 regen
...
let regenned = 0, skipped = 0, missing = 0, errors = 0;
const updates = {};
...
for (const a of allFoods) {
  for (const b of allFoods) {
    ...
    let newNote;
    try { newNote = fxfGen.generate(a, b, tier, ctx); }
    catch (err) { errors++; continue; }
    if (!newNote) { errors++; continue; }
    updates[fwd] = newNote;
    regenned++;
...
console.log('Food×food templated notes regenerated: ' + regenned);
console.log('Editorial preserved (skipped):         ' + skipped);
console.log('Missing key:                           ' + missing);
console.log('Errors:                                ' + errors);
console.log('');
console.log('By archetype:');
for (const [arch, n] of Object.entries(archetypeCounts).sort((a,b)=>b[1]-a[1])) {
  console.log('  ' + arch + ': ' + n);
}
...
const backup = path.join(repo, 'pairing-notes.js.pre-engine-v4-regen.bak');
