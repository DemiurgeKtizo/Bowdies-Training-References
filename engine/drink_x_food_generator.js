...
const BOTTLE_PROFILE = {
  ...
  'Bulleit Bourbon': {
    tastingNotes: ['high-rye', 'spice-forward', 'oak'],
    character:    'high-rye spice-forward depth',
    bridge1:      'the high-rye spice cuts {foodTarget}',
    bridge2:      'the oak underlines {foodSubj}',
    verdictHook:  'high-rye bourbon',
  },
  'Knob Creek': {
  ...
  'Pommery Cuvée Louise': {
    ...
    bridge1:      'the prestige bubbles lift {foodTarget}',
    bridge2:      'the brioche edge frames {foodSubj}',
    verdictHook:  'prestige cuvée Champagne',
  },
  // ── Sparkling ───────────────────────────────────────────────────────────
...
const DRINK_CLASS_DEFAULT = {
  BIG_RED:          { character: 'concentrated dark-fruit and tannin',  bridge1: 'the bold red body wraps {foodTarget}',  bridge2: 'the cassis frames {foodSubj}', verdictHook: 'big-red Cab' },
  ELEGANT_RED:      { character: 'red-fruit-and-spice elegance',         bridge1: 'the silky red-fruit body wraps {foodTarget}', bridge2: 'the cherry-fruit frames {foodSubj}', verdictHook: 'elegant red' },
  BOURBON_BOLD:     { character: 'oak-and-vanilla bourbon depth',        bridge1: 'the bourbon depth settles into {foodTarget}', bridge2: 'the vanilla softens {foodSubj}', verdictHook: 'bourbon' },
  TEQUILA_BOLD:     { character: 'aged-agave character',                 bridge1: 'the aged-agave depth handles {foodTarget}', bridge2: 'the oak frames {foodSubj}', verdictHook: 'añejo tequila' },
  MEZCAL:           { character: 'smoky agave character',                bridge1: 'the mezcal smoke meets {foodTarget}', bridge2: 'the agave threads {foodSubj}', verdictHook: 'smoky mezcal' },
  COGNAC:           { character: 'barrel-aged cognac character',         bridge1: 'the cognac depth wraps {foodTarget}', bridge2: 'the caramel rounds {foodSubj}', verdictHook: 'classic cognac' },
  COGNAC_LUXURY:    { character: 'luxury-icon cognac depth',             bridge1: 'the icon-cognac depth wraps {foodTarget}', bridge2: 'the caramel rounds {foodSubj}', verdictHook: 'luxury cognac' },
  SPARKLING:        { character: 'bright sparkling effervescence',       bridge1: 'the bubbles lift {foodTarget}', bridge2: 'the citrus frames {foodSubj}', verdictHook: 'sparkling' },
  ...
  GIN:              { character: 'botanical lift',                       bridge1: 'the botanical lift frames {foodTarget}', bridge2: 'the juniper threads {foodSubj}', verdictHook: 'gin' },
  VODKA:            { character: 'clean vodka neutrality',               bridge1: 'the vodka neutrality sits with {foodTarget}', bridge2: 'the clean profile carries {foodSubj}', verdictHook: 'vodka' },
  LIGHT_SPIRIT:     { character: 'clean light-spirit character',         bridge1: 'the clean spirit sits with {foodTarget}', bridge2: 'the light body lifts {foodSubj}', verdictHook: 'light spirit' },
  HEAVY_SPIRIT:     { character: 'heavy-spirit weight',                  bridge1: 'the spirit weight handles {foodTarget}', bridge2: 'the depth frames {foodSubj}', verdictHook: 'heavy spirit' },
  COCKTAIL_BOLD:    { character: 'spirit-forward cocktail register',     bridge1: 'the spirit-forward depth handles {foodTarget}', bridge2: 'the cocktail spice frames {foodSubj}', verdictHook: 'spirit-forward cocktail' },
  COCKTAIL_LIGHT:   { character: 'citrus-and-bright cocktail lift',      bridge1: 'the cocktail lift frames {foodTarget}', bridge2: 'the citrus cuts {foodSubj}', verdictHook: 'light cocktail' },
  SWEET_LIQUEUR:    { character: 'sweet liqueur character',              bridge1: 'the sweet character mirrors {foodTarget}', bridge2: 'the liqueur depth threads {foodSubj}', verdictHook: 'digestif liqueur' },
  APERITIVO_BITTER: { character: 'bitter-herb aperitivo edge',           bridge1: 'the bitter herb edge cuts {foodTarget}', bridge2: 'the aperitivo character frames {foodSubj}', verdictHook: 'aperitivo' },
  SWEET_WINE:       { character: 'dessert-wine sweetness',               bridge1: 'the dessert-wine sweetness mirrors {foodTarget}', bridge2: 'the honey threads {foodSubj}', verdictHook: 'dessert wine' },
};
...
function _editorialVerdict(tier, drink, food, profile) {
  ...
  let entries = (tierBucket[v9dc] || {})[v9fc];
  if (!entries || !entries.length) {
    // Try DEFAULT drink class as fallback
    entries = (tierBucket.DEFAULT || {})[v9fc];
  }
  ...
  const FOOD_NOUN_RX = /\b(marrow|crab|cream|potato|shellfish|brulee|cake|fries|fish|chicken|halibut|tuna|trout|salmon|seabass|burrata|escargot|tartare|asparagus|broccoli|spinach|carrot|brownie|cheesecake|beignets|wedge|caesar|gumbo|chowder|bisque|ribeye|filet|steak|tomahawk|pork|belly|sprouts|mushroom|shrimp|cocoa|chocolate|custard|raisin|umami|broth|cheddar|gruyere|honey-glazed|sugar)\b/gi;
  pool = pool.filter(e => {
  ...
  const DRINK_CLASS_KEYWORDS = {
    ...
    APERITIVO_BITTER:['amaro','amari','aperitivo','vermouth','fernet','aperol','campari','friulian'],
  ...
  // by a name fragment that's not the current drink.
  const otherBottlePatterns = [
    'fiddich','glenfiddich','macallan','oban','lagavulin','laphroaig',
    'bowmore','glenmorangie','glenlivet','aberlour','cragganmore',
    'jameson','redbreast','tullamore','bushmills','crown royal',
    'jack daniel','yamazaki','hibiki','hakushu','pappy','stagg',
    'blanton',"booker",'eagle rare','weller','caymus','opus one',
    'silver oak','jordan','far niente','fernet','aperol','campari',
    'amaro nonino','green chartreuse','inverroche','mahón','mahon',
    'long road','bacardi','ron zacapa','don julio','patron','clase azul',
    'cape-floral','gr-pride','michigan-pride',
  ];
...
function generate(drink, food, tier, ctx) {
  ...
  // Tier correction: v9 sometimes overrides the pair-map tier with the actual
  // verdict word (e.g. JW Blue x Bone Marrow is in the gold list but v9 wrote
  // "Works"). Apply mined corrections.
  ...
  if (tier === 'avoid') {
    ...
    return drinkPossessive(drink.name) + ' ' + avoidChar + " overpowers " + foodPossessive(food) + ' ' + foodCharacter(food) + ' -- the plate deserves ' + altsList + ', not ' + article + ' ' + dcLabel + '. Avoid; Reach for ' + altsList + '. Save the ' + drink.name + ' for the steak.';
  }
...
module.exports = {
  ...
  whiskeyVoiceSubclass,
  BOTTLE_PROFILE, DRINK_CLASS_DEFAULT, VERDICT_PATTERNS, WHISKEY_VOICE_DEFAULTS,
};
