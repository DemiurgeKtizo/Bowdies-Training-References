// Generates the 433 remaining food×food pair-notes (cross-cell gaps).
'use strict';
const fs = require('fs');
const vm = require('vm');
...
function processPair(lhsBucket, rhsBucket, label) {
  let lAdded = 0;
  for (const a of lhsBucket) {
    for (const b of rhsBucket) {
      ...
      if (!tier) continue;
      let note;
      try { note = fxfGen.generate(a, b, tier, ctx); }
      catch (err) { console.log('  [ERR] ' + fwd + ': ' + err.message); errors++; continue; }
  ...
  console.log('  ' + label + ': ' + lAdded);
}

processPair(STARTERS, SOUPS,    'Starter × Soup    ');
processPair(STARTERS, SALADS,   'Starter × Salad   ');
processPair(STARTERS, SIDES,    'Starter × Side    ');
processPair(SOUPS,    SALADS,   'Soup × Salad      ');
processPair(SOUPS,    SIDES,    'Soup × Side       ');
processPair(SALADS,   SIDES,    'Salad × Side      ');
processPair(SIDES,    DESSERTS, 'Side × Dessert    ');
...
const sampleKeys = Object.keys(updates).slice(0, 4);
...
const backup = path.join(repo, 'pairing-notes.js.pre-remaining-food-backfill.bak');
