...
const TEMPLATED_SIGS = [
  ...
  // Retired verdict hooks (kept in sync with regenerate_food_x_food.js).
  // Without these, stale templated notes leak into the editorial corpus and
  // recycle their (old, parallel-clause) phrasing into future regen output.
...
// Build output. Format mirrors the existing engine/food_corpus_mined.js
// exactly so consumers (the FxF generator) read the same shape on every regen.
const outPath = path.join(__dirname, 'food_corpus_mined.js');
...
const header = '// engine/food_corpus_mined.js\n' +
