// Generates the 98 missing entree × side pair-notes via the food×food generator.
'use strict';
const fs = require('fs');
const vm = require('vm');
...
const ENTREE_RE = /^(STEAK|FISH_MAIN|CHICKEN|VEG_MAIN)/;
const entrees = ctx.PAIRING_MAP.filter(e => ENTREE_RE.test(taxonomy.foodClassFor(e) || ''));
const sides = ctx.PAIRING_MAP.filter(e => /SIDE/.test(taxonomy.foodClassFor(e) || ''));
...
for (const en of entrees) {
  for (const sd of sides) {
    const fwd = en.name + '|' + sd.name;
    const rev = sd.name + '|' + en.name;
    if (ctx.PAIRING_NOTES[fwd] || ctx.PAIRING_NOTES[rev]) continue;
    const tier = tierByKey.get(fwd) || tierByKey.get(rev);
    if (!tier) continue;
    let note;
    try {
      note = fxfGen.generate(en, sd, tier, ctx);
    } catch (err) {
      console.log('  [ERR] ' + en.name + ' x ' + sd.name + ': ' + err.message);
      errors++;
      continue;
    }
...
console.log('Generated ' + added + ' new entree×side pair-notes (' + Object.keys(updates).length + ' keys with mirrors)');
console.log('Errors: ' + errors);
console.log('');

const sampleKeys = Object.keys(updates).slice(0, 6);
...
const backup = path.join(repo, 'pairing-notes.js.pre-entree-side-backfill.bak');
